...
def cascade(
    ...
    n2: Optional[float] = typer.Option(
        None, "--n2", help="Kerr coefficient n2 (m^2/W) override"
    ),
    a_eff: float = typer.Option(
        0.6e-12, "--a-eff", help="Effective mode area A_eff (m^2) for physics mode"
    ),
    ...
    aeff_um2: Optional[float] = typer.Option(
        None, "--aeff-um2", help="Effective mode area [um^2]"
    ),
    ...
    report: str = typer.Option(
        "none", "--report", help="Report type: 'none' or 'power'"
    ),
    ...
    link_length_um: float = typer.Option(50.0, "--link-length-um", help="Link length per stage [um]"),
    ...
    extinction_target_dB: float = typer.Option(20.0, "--extinction-target-dB", help="Target extinction ratio [dB]"),
    L_eff_um: float = typer.Option(10.0, "--L-eff-um", help="Effective interaction length [um]"),
    # Output options
    save_primary: Optional[Path] = typer.Option(None, "--save-primary", help="Save cascade JSON to file"),
    save_report: Optional[Path] = typer.Option(None, "--save-report", help="Save power report JSON to file"),
    csv: Optional[Path] = typer.Option(None, "--csv", help="Save power report as CSV"),
    embed_report: bool = typer.Option(False, "--embed-report", help="Embed power report in cascade JSON"),
    ...
    n2_resolved = n2 if n2 is not None else (platform_obj.nonlinear.n2_m2_per_W if platform_obj else 1e-17)
    loss_dB_cm_resolved = loss_dB_cm if loss_dB_cm is not None else (platform_obj.fabrication.loss_dB_per_cm if platform_obj else 0.1)
...
@app.command("sweep")
def sweep(
    platforms: List[str] = typer.Option(["Si", "SiN", "AlGaAs"], "--platforms", help="Material platforms to sweep"),
    ...
    P_high_mW: List[float] = typer.Option([0.3, 0.5, 1.0], "--P-high-mW", help="Drive powers [mW] to sweep"),
    fanout: List[int] = typer.Option([1, 2], "--fanout", help="Fan-out values to sweep"),
    coupling_eta: List[float] = typer.Option([0.8], "--coupling-eta", help="Coupling efficiency values"),
    link_length_um: List[float] = typer.Option([50.0], "--link-length-um", help="Link lengths [um]"),
    # Single-value parameters
    lambda_nm: Optional[float] = typer.Option(None, "--lambda-nm", help="Operating wavelength [nm]"),
    ...
    # Output options
    outdir: Optional[Path] = typer.Option(None, "--outdir", help="Directory for per-point JSON files"),
    ...
    show_config: bool = typer.Option(False, "--show-config", help="Show sweep configuration and exit"),
    # Parallel processing
    ...
    for idx, combo in enumerate(combinations):
        ...
        try:
            ...
            wavelength_nm_resolved = params.get("lambda_nm") or (platform_obj.default_wavelength_nm if platform_obj else 1550.0)
            Aeff_um2_resolved = params.get("aeff_um2") or (platform_obj.nonlinear.Aeff_um2_default if platform_obj else 0.6)
            n2_resolved = params.get("n2") if params.get("n2") is not None else (platform_obj.nonlinear.n2_m2_per_W if platform_obj else 1e-17)
            loss_dB_cm_resolved = params.get("loss_dB_cm") if params.get("loss_dB_cm") is not None else (platform_obj.fabrication.loss_dB_per_cm if platform_obj else 0.1)
            ...
            ctl = ExperimentController(dev)
            cascade_result = ctl.test_cascade(n_stages=2, threshold_mode=params["threshold"], beta=params["beta"])
            ...
            pins = PowerInputs(
                ...
                beta_2pa_m_per_W=(platform_obj.nonlinear.beta_2pa_m_per_W if platform_obj else 0.0),
                auto_timing=params.get("auto_timing", False) or (params.get("pulse_ns") is None and params.get("bitrate_GHz") is None)
            )
            ...
            if not quiet:
                typer.echo(f"[sweep] {idx+1}/{len(combinations)} done: platform={params.get('platform')} P={params['P_high_mW']}mW β={params['beta']} fanout={params['fanout']}")
