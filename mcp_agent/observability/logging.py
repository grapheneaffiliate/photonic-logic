...
def sanitize_sensitive_data(logger: FilteringBoundLogger, method_name: str, event_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    def sanitize_dict(d: dict[str, Any]) -> dict[str, Any]:
        sanitized = {}
        for k, v in d.items():
            # Convert bytes keys to strings for comparison
            ...
            if any(sensitive in key_str.lower() for sensitive in sensitive_keys):
                ...
                # Handle lists (like headers which come as list of tuples)
                sanitized[k] = [sanitize_dict(dict([item])) if isinstance(item, tuple) else item for item in v]
            elif isinstance(v, bytes):
                # Convert bytes values to strings
                sanitized[k] = v.decode('utf-8', errors='ignore')
            elif isinstance(v, str) and len(v) > 20 and any(sensitive in key_str.lower() for sensitive in sensitive_keys):
...
class LoggingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Convert headers from list of byte tuples to dict of strings
        headers = {}
        for name, value in scope.get("headers", []):
            headers[name.decode('utf-8', errors='ignore')] = value.decode('utf-8', errors='ignore')
