...
@dataclass
class Params:
    kA: float; kB: float; J: float
    k_eA: float; k_eB: float
    detA0: float; detB0: float
    gXPM: float
...
def cm_transfer(params: Params, Pin_ctrl_W: float, dA_bias=0.0, dB_bias=0.0) -> Dict[str,float]:
    ...
    M = np.array([[j*dA - params.kA/2, -1j*params.J],
                  [-1j*params.J,       j*dB - params.kB/2]], dtype=complex)
...
if __name__ == "__main__":
    GHz = TWOPI*1e9
    params = Params(kA=0.39*GHz, kB=0.39*GHz, J=1.5*GHz,
                    k_eA=0.20*GHz, k_eB=0.20*GHz,
                    detA0=0.0, detB0=0.0, gXPM=2.5*GHz/1e-3)
