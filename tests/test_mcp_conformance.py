...
# Try to use streamable HTTP client, fall back to regular HTTP client
try:
...
class TestMCPConformance:
    ...
    async def test_list_tools_schema_validity(self):
        ...
        pytest.skip("Requires running MCP server - use test_mcp_contract.py instead")
        
        url = MCP_SERVER_URL if MCP_SERVER_URL.endswith("/mcp") else f"{MCP_SERVER_URL}/mcp"
        async with http_client(url) as (read, write, _):
            async with ClientSession(read, write) as session:
                ...
                for tool in tools:
                    ...
                    # Validate JSON Schema using Draft7 (compatible with jsonschema 4.x)
                    try:
    ...
    def test_tool_schemas_are_complete(self):
        ...
        # Should not raise an exception (use Draft7 for compatibility)
        jsonschema.Draft7Validator.check_schema(sample_schema)
