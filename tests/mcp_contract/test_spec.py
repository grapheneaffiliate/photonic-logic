...
@pytest.mark.asyncio
@pytest.mark.skip(reason="stdio_client requires a server process to be running")
async def test_mcp_contract():
    """Test MCP server contract compliance."""
    # This test requires a running MCP server process
    # In CI, we test via HTTP endpoints instead
    server_script = "python -m mcp_agent.server"
